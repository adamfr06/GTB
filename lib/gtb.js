...
const copperBulbGenerationFacts = Object.fromEntries(
  [
    ...
    id,
    {
      categories: ["block", "mineral", "redstone", "light"],
      naturalGeneration: {
        generated: true,
        places: ["trial chambers"],
        note: "Copper bulbs can appear as placed blocks in generated trial chambers."
      },
      generation: {
...
const extraBlockFacts = {
  ...
  "minecraft:lectern": {
    categories: ["utility", "wood"],
    materialNotes: "Wood-related: crafted from wooden slabs and a bookshelf, has a wooden texture, and is fastest to break with an axe.",
    naturalGeneration: {
      generated: true,
      places: ["villages"],
      note: "Can appear as a placed job-site block in generated villages."
    },
    generation: {
      terrainOrFeature: false,
      structurePlaced: true
    },
    shapeNotes: "Not a full 1x1x1 cube; it has a lectern shape with empty space around the stand."
  }
};
...
export async function createGame(requestedBlockId) {
  ...
  if (useSupabase()) {
    await dbInsert("games", gameToRow(game));
...
export async function askQuestion(gameId, rawQuestion) {
  ...
  if (useSupabase()) {
    ...
    const questions = await appendGameQuestion(game.id, questionRecord);
...
export async function createReport(body) {
  ...
  if (useSupabase()) {
    await dbInsert("reports", reportToRow(report));
    return report;
  }
...
export async function decideReport(reportId, decision, reviewer = "admin") {
  ...
  if (useSupabase()) {
    ...
    if (decision === "approved") {
      ...
      await dbInsert("corrections", correctionToRow(correction));
    }

    await dbUpdate("reports", report.id, reportToRow(report));
...
async function answerWithAi(block, question, corrections) {
  ...
  const prompt = [
    ...
    "Do not use code tools for color, texture, material, crafting, recipe resource counts, natural generation, shape/full-cube status, light, sound, trading, or gameplay property questions.",
    ...
    "You may use reliable Minecraft Wiki/general Minecraft knowledge to answer common properties not explicitly listed, such as whether a block naturally generates, emits light, is crafted, is a full cube, or appears in a dimension.",
    "Do not treat local 'solid' as meaning full cube. Full block/full cube means the block occupies the complete 1x1x1 cube shape.",
  ...
  for (const model of geminiModels) {
    ...
    if (result.ok) {
      ...
      if (isToolRequest(parsed)) {
        lastInvalidTool = parsed;
        const retry = await callGemini(model, invalidToolRetryPrompt(prompt, parsed));
        if (retry.ok) {
          const retryParsed = safeParseJson(retry.text);
          const retryToolAnswer = runAnswerTool(block, retryParsed, question);
          if (retryToolAnswer) return retryToolAnswer;
          if (!isToolRequest(retryParsed)) {
            return cleanAiAnswer(retryParsed, wikiContext.available ? "gemini+wiki" : "gemini");
          }
          lastInvalidTool = retryParsed;
        } else {
          lastError = retry;
          console.error(`Gemini retry error (${model}):`, retry.text);
          continue;
        }
      }
      if (isToolRequest(parsed)) continue;
  ...
  if (isGeminiQuotaError(lastError)) {
    throw new HttpError(429, "Gemini quota was hit. Your question was not counted. Wait a minute, then ask again.");
  }

  if (lastInvalidTool) {
    throw new HttpError(422, "Gemini misclassified this question. It was not counted. Try rephrasing it.");
  }
...
function deterministicAnswer(block, question) {
  ...
  const asksLetterInBlockName = /\bletter\b/.test(q) && /\b(block|it|this)\b/.test(q);

  if (letterMatch && (asksAboutName || asksAboutId || asksLetterInBlockName)) {
  ...
  const wordCountMatch = q.match(/\b(?:is it|is this|does it have|has it got)\s+(\d+)\s+words?\b/) ||
    q.match(/\b(\d+)\s+words?\b/);
  if (wordCountMatch) {
    const expected = Number(wordCountMatch[1]);
    if (Number.isFinite(expected)) {
      return {
        answer: words.length === expected ? "yes" : "no",
        confidence: 1,
        reason: `Checked the exact block name directly: ${facts.name} has ${words.length} word(s).`,
        source: "code"
      };
    }
  }

  const letterCountMatch = q.match(/\b(\d+)\s+letters?\b/);
  if (letterCountMatch && /\b(name|word|called|it|block)\b/.test(q)) {
    const target = facts.name.replace(/[^a-z0-9]/gi, "");
    const expected = Number(letterCountMatch[1]);
    const actual = target.length;
    const asksAtLeast = /\b(more than or equal|greater than or equal|at least|>=|minimum)\b/.test(q);
    const asksMore = /\b(more than|greater than|over|>)\b/.test(q) && !asksAtLeast;
    const asksAtMost = /\b(less than or equal|fewer than or equal|at most|<=|maximum)\b/.test(q);
    const asksLess = /\b(less than|fewer than|under|<)\b/.test(q) && !asksAtMost;
    const value = asksAtLeast
      ? actual >= expected
      : asksMore
        ? actual > expected
        : asksAtMost
    ...
    return {
      answer: value ? "yes" : "no",
      confidence: 1,
      reason: `Checked the exact block name directly: ${facts.name} has ${actual} letter(s), ignoring spaces and punctuation.`,
      source: "code"
    };
  }
...
function localFactAnswer(block, question) {
  ...
  const asksCraftable =
    /\b(is it|is this|can it be|is the block|is this block)\b.{0,30}\b(craftable|crafted)\b/.test(q) ||
    /\b(can you craft|can i craft)\b/.test(q);
  if (asksCraftable) {
    if (/\b(less|fewer|more|greater|equal|at least|at most|under|over|\d+|resource|resources|ingredient|ingredients|recipe)\b/.test(q)) {
      return null;
    }
    return {
  ...
  if (asksNatural) {
    ...
    const hasPositiveEvidence = excludesStructures
      ? facts.generation?.terrainOrFeature === true
      : facts.naturalGeneration?.generated === true;

    if (hasPositiveEvidence) {
      return {
        answer: "yes",
        confidence: 0.92,
        reason: `Checked positive local generation facts for ${facts.name}: ${facts.naturalGeneration?.note || "generates naturally"}`,
        source: "code"
      };
    }
  }
...
function extractAskedColor(question) {
  ...
  return colorWords.find((color) => new RegExp(`\\b${escapeRegExp(color)}\\b`).test(q)) || null;
}
...
function isToolRequest(parsed) {
  return Boolean(parsed && typeof parsed === "object" && parsed.tool);
}

function invalidToolRetryPrompt(originalPrompt, parsed) {
  return [
    originalPrompt,
    "",
    "Your previous response was rejected because it requested a code tool for a question that is not a direct string/name/letter/word-count question.",
    `Rejected tool request: ${JSON.stringify(parsed)}`,
    "Do not request a tool this time.",
    "Answer the original player question normally as a Minecraft recipe/color/material/gameplay question.",
    "Return only valid JSON with keys: answer, confidence, reason."
  ].join("\n");
}
...
function heuristicAnswer(block, question) {
  ...
  if (excludesStructures && (q.includes("natural") || q.includes("spawn") || q.includes("generate"))) {
    ...
    return {
      ...
      confidence: 0.78,
      reason: "Local fallback treated this as natural terrain/feature generation, excluding placed blocks in generated structures.",
      source: "fallback"
    };
  }
...
function isGeminiQuotaError(error) {
  ...
  return error?.status === 429 || /\b(quota|rate limit|rate-limit|resource_exhausted)\b/i.test(message || text);
}
...
function expandBlockFacts(block) {
  ...
  const categories = uniqueList([...(patchedBlock.categories || []), ...(extra.categories || [])]);
  const generation = extra.generation ||
    patchedBlock.generation || {
      terrainOrFeature: Boolean(categories.includes("natural")),
      structurePlaced: Boolean(extra.naturalGeneration?.generated && !categories.includes("natural"))
    };
  const naturalGeneration =
    extra.naturalGeneration ||
    patchedBlock.naturalGeneration || {
      generated: Boolean(categories.includes("natural")),
      places: categories.includes("natural") ? patchedBlock.dimensions || [] : [],
      note: categories.includes("natural")
        ? "Generates as part of terrain, fluids, or natural world features."
        : "No known natural generation in the local database."
    };

  return { ...patchedBlock, ...extra, categories, generation, naturalGeneration };
}
...
async function relevantCorrections(blockId) {
  if (useSupabase()) {
    ...
    if (error) throw new HttpError(500, `Supabase corrections read failed: ${error.message}`);
    return (data || []).map(rowToCorrection);
  }
...
async function findGame(gameId) {
  if (useSupabase()) {
    ...
    if (error) throw new HttpError(500, `Supabase game read failed: ${error.message}`);
...
async function updateGame(game) {
  if (!useSupabase()) throw new HttpError(500, "Supabase is not configured.");
  await dbUpdate("games", game.id, gameToRow(game));
}

async function appendGameQuestion(gameId, questionRecord) {
  if (!useSupabase()) throw new HttpError(500, "Supabase is not configured.");
  const { data, error } = await supabase().rpc("append_game_question", {
    p_game_id: gameId,
    p_question: questionRecord
  });
  if (error) {
    if (error.message?.includes("game_not_updateable")) {
      throw new HttpError(400, "This game is already over or has no questions left.");
    }
    throw new HttpError(500, `Supabase question append failed: ${error.message}`);
  }
...
async function listReports() {
  ...
  if (error) throw new HttpError(500, `Supabase reports read failed: ${error.message}`);
  return (data || []).map(rowToReport);
}

async function getReport(reportId) {
  const { data, error } = await supabase().from("reports").select("*").eq("id", reportId).maybeSingle();
  if (error) throw new HttpError(500, `Supabase report read failed: ${error.message}`);
...
async function dbUpdate(table, id, row) {
  ...
  if (error) throw new HttpError(500, `Supabase update failed: ${error.message}`);
}
...
function gameToRow(game) {
  return {
    id: game.id,
    hidden_block_id: game.hiddenBlockId,
    status: game.status,
...
function rowToGame(row) {
  return {
    id: row.id,
    hiddenBlockId: row.hidden_block_id,
    status: row.status,
...
function reportToRow(report) {
  return {
    ...
    question: report.question,
    ai_answer: report.aiAnswer,
    suggested_answer: report.suggestedAnswer,
...
function correctionToRow(correction) {
  return {
    id: correction.id,
    ...
    source_report_id: correction.sourceReportId,
